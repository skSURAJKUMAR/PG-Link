...
def dbconnect():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM IND_PGLINK_FROM_DATE")
    xt=cursor.fetchall()
    # print(xt)
    ...
    Query=f"exec PGLINKDATA \'2\',\'{fromDate}\',\'{toDate}\'" 
    print(Query)    #1 for grp & 2 for ind
    ...
    if(len(x)>0):
        try:
            API_Call(x,URL)
            updateQuery=f"UPDATE IND_PGLINK_FROM_DATE SET fromDate = '{str(toDate)}'"
            print(updateQuery)
...
try:
    ...
    save_path = 'D:\\PGLinkGenerator\\IND_Logs'
    if not os.path.exists(save_path):
            os.mkdir(save_path)
    completeName = save_path+"\\"+'IND_log_'+logdate+'.txt'
